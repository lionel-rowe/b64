...
const toUtf8Bin = char => {
  ...
  if (!rule) throw new RangeError(`Invalid Unicode codepoint: \`0x${codepoint.toString(16)}\``);
...
const toUtf8Arr = str => {
  ...
  Array.from(str).forEach(char => out = out.concat(toUtf8Bin(char)));
...
const fromUtf8Arr = utf8Arr => {
  ...
  utf8Arr.forEach(byte => {
    if (trailingByteCounter < 0) {
      ...
      if (!rule) throw new RangeError(`Invalid UTF-8 byte at index \`${i}\``);
    }
    ...
    if ((header << dataLength & LEADING_ONES[8 - dataLength]) !== (byte & LEADING_ONES[8 - dataLength])) {
      throw new RangeError(`Invalid UTF-8 byte at index \`${i}\``);
    }
    ...
    if (trailingByteCounter === 0) {
      outStr = outStr.concat(String.fromCodePoint(outCodePoint))
    }
...
const toB64 = bytes => {
  ...
  while (bytes.length) {
    ...
    sextets.push(octets[0] >> 2);

    sextets.push((octets[0] & TRAILING_ONES[2]) << 4);

    if (typeof octets[1] !== 'undefined') {
      sextets[1] |= octets[1] >> 4;
      sextets.push((octets[1] & TRAILING_ONES[4]) << 2);
    } else sextets.push(-1);

    if (typeof octets[2] !== 'undefined') {
      sextets[2] |= octets[2] >> 6;
      sextets.push(octets[2] & TRAILING_ONES[6]);
    } else sextets.push(-1);

    sextets.forEach(sextet => b64 += sextet === -1 ? '=' : B64_CHARS[sextet]);
  }
...
const fromB64 = b64 => {
  ...
  const b64Bin = b64.split('').map((char, idx) => {
    ...
    if (typeof val === 'undefined') {
      throw new RangeError(`Invalid Base64 character: ${char} at index: ${idx}`)
    }
  ...
  while (b64Bin.length) {
    ...
    octets.push(sextets[0] << 2);

    if (typeof sextets[1] !== 'undefined') {
      octets[0] |= sextets[1] >> 4;
      octets.push((sextets[1] & TRAILING_ONES[4]) << 4);
    }
    ...
    if (typeof sextets[2] !== 'undefined') {
      ...
      octets.push((sextets[2] & TRAILING_ONES[2]) << 6);
    }
